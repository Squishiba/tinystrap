...
Steps:
- [ ] Write failing test `packages/policy/test/types.test.ts`:
...
- [ ] Run `pnpm test -- types` (from repo root; vitest filter). Expected: FAIL — types not exported.
- [ ] Create `packages/policy/src/types.ts` with the exact code above; update `packages/policy/src/index.ts` to `export * from "./types.js";`.
- [ ] Run `pnpm test -- types`. Expected: PASS (2 tests).
- [ ] Commit:
