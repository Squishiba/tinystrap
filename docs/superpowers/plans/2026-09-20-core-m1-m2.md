...
Steps:
- [ ] Write failing test `packages/policy/test/pathpolicy.test.ts`:
...
- [ ] Run `pnpm test -- pathpolicy`. Expected: FAIL — not exported.
- [ ] Implement `packages/policy/src/pathpolicy.ts`:
...
- [ ] Add `export * from "./pathpolicy.js";` to `index.ts`. Run tests. Expected: PASS (4 tests).
- [ ] Commit:
