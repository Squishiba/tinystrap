...
Steps:
- [ ] Write failing test `packages/policy/test/guards.test.ts`:
...
- [ ] Run `pnpm test -- guards`. Expected: FAIL — not exported.
- [ ] Implement `packages/policy/src/guards.ts`:
...
- [ ] Add `export * from "./guards.js";` to `index.ts`. Run tests. Expected: PASS (3 tests).
- [ ] Commit:
