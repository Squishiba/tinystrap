...
- [ ] Create `.gitignore` (append if present): `node_modules/`, `dist/`, `.tinystrap/`.
...
- [ ] Create `packages/policy/tsconfig.json`, `packages/core/tsconfig.json`, `packages/discovery/tsconfig.json`, `apps/cli/tsconfig.json`, each:
...
- [ ] Create `packages/discovery/package.json` and `packages/core/package.json` and `apps/cli/package.json` following the same shape, with dependencies: core → `"@tinystrap/policy": "workspace:*"`, `"@tinystrap/discovery": "workspace:*"`, `"smol-toml": "^1.3.0"`; cli → `"@tinystrap/core": "workspace:*"`, `"@tinystrap/discovery": "workspace:*"`; discovery → none.

- [ ] Create placeholder `src/index.ts` in policy, discovery, core (each: `export {};`) and `apps/cli/src/main.ts` (`console.log("tinystrap");`).

- [ ] Create failing smoke test `packages/policy/test/smoke.test.ts`:
...
- [ ] Run `pnpm install` (timeout ≥ 120s), then `pnpm test`. Expected: 1 test PASS (the smoke test as written passes once exports resolve; if it fails on resolution, fix workspace wiring before continuing).
- [ ] Commit:
