...
Steps:
- [ ] Write failing test `packages/policy/test/registry.test.ts`:
...
- [ ] Write failing test `packages/policy/test/capability.test.ts`:
...
- [ ] Run `pnpm test -- registry capability`. Expected: FAIL — functions not exported.
- [ ] Implement `packages/policy/src/registry.ts`:
...
- [ ] Implement `packages/policy/src/capability.ts`:
...
- [ ] Add `export * from "./registry.js"; export * from "./capability.js";` to `index.ts`. Run tests. Expected: PASS (4 tests).
- [ ] Commit:
